...
class BtcTradeUA(object):
    ...
    def __get_request(self, url, raw_data=None, auth=False):
        ...
        if auth:
            custom_headers = {
                ...
                "api_sign":
                    BtcTradeUA.make_api_sign(self.__private_key, raw_data)
            }

        result = requests.get(url, params=raw_data,
    ...
    def __post_request(self, url, raw_data=None, auth=True, auth2fa=False):
        custom_headers = None
        if auth:
            custom_headers = {
    ...
    def send(self, currency, wallet, amnt, payment_id=None, out_order_id=None):
        ...
        params = {"out_order_id": out_order_id, "payment_id": payment_id, "nonce": self.__nonce, "currency": currency,
                  "wallet": wallet, "amnt": amnt}
        raw_data = urllib.urlencode(params)
